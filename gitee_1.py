...
class Model(dict):
    def __init__(self, desc,time,watch,star,fork, name, url, label): 
        self['desc'] = desc 
        self['time'] = time 
        self['watch'] = watch
        self['star'] = star 
...
def fetch_one(i, page):
    result = fetch_one_only(i, page)
    for m in result:
        logger.info(f"name = {m.name} url = {m.url} type = {m.label} \ndesc = {m.desc}update-time = {m.time}\nwatch_num = {m.watch} star_num = {m.star} fork_num = {m.fork}\n")
        if(q.full()):
            logger.warn("full")
        q.put(m)

def fetch_one_only(i, page):
    resp = Get(f"https://gitee.com/search?page={page}&q={i}&sort=stars_count&type=")
    sp = Soup(resp.text, "lxml")
    result = extract_data(sp)
...
if __name__ == "__main__":
    ...
    futures_to_page = {executor.submit(fetch_all_q,q) : q for q in s}

    for i in concurrent.futures.as_completed(futures_to_page):
        i.result()

    futures_to_page = {executor.submit(fetch_all_q,chr(q)) : q for q in range(ord('a'), ord('z') + 1)}

    for i in concurrent.futures.as_completed(futures_to_page):
        i.result()
    #fetch_all_q("sdjfisodfjasoidfjasodifjasf")
